...
final class ProductContainerViewController: UIViewController {
    ...
    private func setupProductCollectionViewController(dataSource: [Produto]) {
        ...
        let datasourceController: DatasourceController<ProductFacade, ProductCollectionViewCell> = .build(for: dataSource)
        productCollectionViewController.productDatasourceController = datasourceController
        productCollectionViewController.sortController = CollectionSortController(datasourceController: datasourceController)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "OfferSortOptionsViewController":
            ...
            self.offerSortOptionsViewController.onPickerRowSelected = { [unowned self] selectedOption in
                self.productCollectionViewController.sortController?.sort(by: CollectionSortOption(rawValue: selectedOption) ?? .popularidade)
            }
